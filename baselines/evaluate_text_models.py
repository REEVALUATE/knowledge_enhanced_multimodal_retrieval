...
def evaluate_text_model(
    ...
    if mode == 'single':
        ...
        logger.info("Computing T2T metrics (single mode: variant 0 → variants 1-4)...")
        
        query_embeddings = text_embeddings_by_variant[0]  # (N, D)
        
        # Build candidate pool: variants 1-4
        candidate_texts = []
        text_to_artifact = []
        ...
        # Multi mode: each variant finds other 4, then average
        logger.info("Computing T2T metrics (multi mode: each variant → other 4, then average)...")
        ...
        for query_v_idx in range(5):
            ...
            # Build candidate pool: other 4 variants
            candidate_texts = []
            text_to_artifact = []
            
            for artifact_idx in range(N):
                for v_idx in range(5):
                    if v_idx != query_v_idx:  # Exclude same variant
                        candidate_texts.append(text_embeddings_by_variant[v_idx][artifact_idx])
                        text_to_artifact.append(artifact_idx)
        ...
        # Average across all 5 query variants
        for k in k_values:
        ...
        logger.info(f"T2T: Each of 5 variants queries N×4 = {N}×4 candidates")
